...
const back = (id) => {
  request.get('/borrow/back/'+id).then(res=>{
    if (res.code==='200'){
...
const delBatch = () => {
  ...
  request.post("/borrow/del/batch", ids).then(res => {
    if (res.code === '200') {
      ...
      <el-table :data="tableData" @selection-change="handleSelectionChange">
        ...
        <el-table-column label="图书名称" align="center">
          <template #default="scope">
            <span v-if="scope.row.bookId">{{ books.find(item => item.id === scope.row.bookId)?.name }}</span>
          </template>
        </el-table-column>
    ...
    <el-dialog v-model="dialogFormVisible" :title="form.id ? '编辑' : '新增'" width="30%" center>
      <el-form :model="form" label-width="100px">
        <el-form-item label="图书">
          <el-select v-model="form.bookId" placeholder="请选择图书" style="width: 220px">
