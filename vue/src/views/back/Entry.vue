...
const join = (id) => {
  request.get('/entry/join/'+id).then(res=>{
    if (res.code==='200'){
      ElMessage.success("活动已打卡参加")
      load()
    }
})
}
...
// 删除
const del = (id) => {
  request.delete("/entry/" + id).then(res => {
    if (res.code === '200') {
...
const delBatch = () => {
  ...
  request.post("/entry/del/batch", ids).then(res => {
    if (res.code === '200') {
...
<!--      <el-button plain type="primary" @click="handleAdd" :icon="Plus">新增</el-button>-->
    ...
    <el-card>
      <el-table :data="tableData" @selection-change="handleSelectionChange">
        ...
        <el-table-column label="活动" width="120" align="center">
        ...
        <el-table-column prop="time" label="参与打卡时间" width="140" align="center"/>
        <el-table-column label="状态" width="100" align="center">
          <template #default="scope">
            <el-tag>{{scope.row.status}}</el-tag>
          </template>
        </el-table-column>
        <el-table-column prop="name" label="报名人姓名"/>
        <el-table-column prop="phone" label="报名人联系电话"/>
        <el-table-column label="打卡"  width="240" align="center">
          <template v-slot:="scope"  width="280">
            <el-button type="primary" v-if="scope.row.status==='待打卡'" @click="join(scope.row.id)">打卡参加</el-button>
          </template>
        </el-table-column>
